"""SageMaker entry point for dense LoRA model evaluation.

Evaluates a fine-tuned dense SentenceTransformer model on IR test sets.
...
def find_model_root(model_path: Path) -> Path:
    """Find the SentenceTransformer model root directory."""
...
def parse_args():
    ...
    parser.add_argument("--eval_baseline", type=str, default="true")
...
def main():
    ...
    if args.eval_baseline.lower() in ("true", "1", "yes"):
        ...
        baseline_model = load_model(baseline_path, max_seq_length=512, sparse=False)
    ...
    trained_model = load_model(str(model_path), max_seq_length=512, sparse=False)
    result = evaluate_model(
