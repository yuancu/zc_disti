"""SageMaker entry point for dense model LoRA fine-tuning.

Supports two loss types:
  - kldiv: DistillKLDivLoss with teacher scores (knowledge distillation)
  - mnrl: MultipleNegativesRankingLoss (contrastive, no teacher scores)

Features:
  - LoRA applied via peft (targets qkv_proj in GTE architecture)
  - Dev set evaluation using InformationRetrievalEvaluator
  - Early stopping based on cosine NDCG@10
  - BestAdapterSaver saves LoRA adapters during training, merges at end
  - Optional per-query score normalization (z-norm + scale factor)
...
from sentence_transformers.losses import DistillKLDivLoss, MultipleNegativesRankingLoss
...
class BestAdapterSaver(TrainerCallback):
    """Save the best LoRA adapter weights on eval improvement.

    During training, saves only the adapter weights (small, fast).
    The main script merges the best adapter into the base model at the end.
    """
    ...
    def on_evaluate(self, args: _TA, state: TrainerState, control: TrainerControl, model=None, metrics=None, **kwargs):
        ...
        if val is not None and val > self.best_metric:
            ...
            if model is not None:
                ...
                print(f"Saving {label} adapter at step {self.best_step}: {self.metric_name}={val:.4f}")
                model[0].auto_model.save_pretrained(self.save_dir)


def load_and_convert(data_dir, num_negatives, loss_type="kldiv", normalize_scores=False,
                     score_scale=20.0, score_scale_mode="multiply"):
    """Load teacher-scored JSON data and convert to training format.

    For kldiv: returns Dataset with columns query, positive, negative1..N, label (tensor).
    For mnrl: returns Dataset with columns anchor, positive, negative1..N (no labels).
    """
    ...
    for ex in all_data:
        ...
        # Score transformation for KLDiv
        if loss_type == "kldiv":
            if score_scale_mode == "znorm" and normalize_scores:
                mu = sum(scores) / len(scores)
                std = max((sum((s - mu) ** 2 for s in scores) / len(scores)) ** 0.5, 1e-8)
                scores = [(s - mu) / std * score_scale for s in scores]
            elif score_scale_mode == "multiply":
    ...
    if loss_type == "mnrl":
        # MNRL uses "anchor" instead of "query"
        data = {"anchor": queries, "positive": positives, **negatives_cols}
    ...
    print(f"Loaded {len(ds)} training examples; skipped {n_skipped} queries "
          f"(loss={loss_type}, normalize={normalize_scores})")
    return ds
...
def load_dev_evaluator(dev_dir):
    ...
    Reads:
      - dev_data.jsonl: {query, positive_id, candidates: [{id, text}]}
      - dev_labels.jsonl: {query, candidate_id, llm_score}
    """
...
def setup_lora(model, r, alpha, dropout, target_modules):
    """Apply LoRA to the SentenceTransformer model."""
...
def parse_args():
    ...
    parser.add_argument("--loss_type", type=str, default="kldiv", choices=["kldiv", "mnrl"])
    parser.add_argument("--normalize_scores", type=str, default="false")
    parser.add_argument("--score_scale", type=float, default=0.025)
    parser.add_argument("--score_scale_mode", type=str, default="multiply", choices=["multiply", "znorm"])
    ...
    parser.add_argument("--lora_target_modules", type=str, default="qkv_proj")
...
def main():
    ...
    normalize = args.normalize_scores.lower() in ("true", "1", "yes")

    print(f"Config: model={args.model_name}, loss_type={args.loss_type}, normalize={normalize}")
    print(f"  lora_r={args.lora_r}, lora_alpha={args.lora_alpha}, target_modules={target_modules}")
    print(f"  batch_size={args.train_batch_size}, lr={args.learning_rate}, max_steps={args.max_steps}")
    ...
    train_dataset = load_and_convert(
        ...
        loss_type=args.loss_type, normalize_scores=normalize,
    ...
    model = setup_lora(model, args.lora_r, args.lora_alpha, args.lora_dropout, target_modules)

    # Build loss
    if args.loss_type == "mnrl":
        loss = MultipleNegativesRankingLoss(model=model)
        print("Using MultipleNegativesRankingLoss")
    else:
        loss = DistillKLDivLoss(model=model)
        print(f"Using DistillKLDivLoss (normalize={normalize}, scale={args.score_scale})")
    ...
    eval_kwargs = {}
    callbacks = []
    best_adapter_dir = "/opt/ml/output/best_adapter"
    adapter_saver = None
    if dev_evaluator is not None:
        ...
        adapter_saver = BestAdapterSaver(save_dir=best_adapter_dir)
        callbacks.append(adapter_saver)
    ...
    if is_main:
        if adapter_saver is not None and adapter_saver.best_step is not None and os.path.isdir(best_adapter_dir):
            print(f"Using best adapter from step {adapter_saver.best_step} "
                  f"(NDCG@10={adapter_saver.best_metric:.4f})")
            # Reload base model, apply best adapter, merge, save
            base_model = SentenceTransformer(args.model_name, trust_remote_code=True)
            base_model.max_seq_length = args.max_seq_length
            base_model[0].auto_model = PeftModel.from_pretrained(
                base_model[0].auto_model, best_adapter_dir
            )
            base_model[0].auto_model = base_model[0].auto_model.merge_and_unload()
            base_model.save_pretrained(args.model_dir)
        else:
            print("No best adapter saved, using final model (merging LoRA)")
        ...
        config = {
            ...
            "best_step": adapter_saver.best_step if adapter_saver else None,
            "best_dev_ndcg10": adapter_saver.best_metric if adapter_saver else None,
        }
        with open(os.path.join(args.model_dir, "training_config.json"), "w") as f:
            json.dump(config, f, indent=2)
        print(f"Saved merged model to: {args.model_dir}")
