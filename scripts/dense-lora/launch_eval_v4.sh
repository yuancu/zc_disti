...
# Launch eval jobs for completed Dense LoRA v4 (rank sweep) training jobs.
...
EOF
tar -czf /tmp/dense_lora_v4_eval_code.tar.gz -C "$CODE_DIR" .
aws s3 cp /tmp/dense_lora_v4_eval_code.tar.gz "s3://${BUCKET}/${S3_PREFIX}/eval-code/sourcedir.tar.gz" --region "$REGION"
rm -rf "$CODE_DIR" /tmp/dense_lora_v4_eval_code.tar.gz
echo "Eval code uploaded."

# ============================================================
# Step 2: Upload eval data (reuse from v3)
# ============================================================
echo ""
...
# ============================================================
# Step 3: Launch eval jobs for completed v4 training jobs
# ============================================================
...
echo ""
echo "========================================"
echo "Launching Dense LoRA v4 eval jobs"
echo "========================================"

# Collect all completed v4 training jobs with pagination
ALL_TRAIN_JOBS=""
NEXT_TOKEN=""
while true; do
    CMD=(aws sagemaker list-training-jobs --region "$REGION" --name-contains "dl-v4" --status-equals Completed --max-results 100 --output json)
...
for TRAIN_JOB in $ALL_TRAIN_JOBS; do
    # Parse: dl-v4-r{rank}-{dataset}-{timestamp}
    SUFFIX="${TRAIN_JOB#dl-v4-}"

    # Extract rank: r4, r8, r32, r64, r128, r256
    RANK_STR=$(echo "$SUFFIX" | grep -oP '^r\d+')
    if [ -z "$RANK_STR" ]; then
        echo "  WARNING: Cannot parse rank from $TRAIN_JOB, skipping"
        SKIPPED=$((SKIPPED + 1))
        continue
    fi

    REST="${SUFFIX#${RANK_STR}-}"

    # Extract dataset (remove timestamp)
    DATASET=$(echo "$REST" | sed 's/-[0-9]\{8\}-[0-9]\{6\}.*$//')
    if [ -z "$DATASET" ] || [ "$DATASET" = "$REST" ]; then
        DATASET=$(echo "$REST" | sed 's/-[0-9]\{8\}-[0-9]*$//')
    fi
    if [ -z "$DATASET" ] || [ "$DATASET" = "$REST" ]; then
        DATASET=$(echo "$REST" | sed 's/-[0-9]\{7,\}$//')
    fi
    ...
    EVAL_CHECK="ev4-${RANK_STR}-${DATASET}"
    ...
    EVAL_JOB_NAME="ev4-${RANK_STR}-${DATASET}-${TIMESTAMP}"
